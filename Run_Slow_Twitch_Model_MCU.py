...
class SlowTwitch():
   ...
   # Ion Concentrations and Nernst Potentials (Sarcolemma)
    ...
    V_MCU      = 0.05*0.0215#float(sys.argv[2]) #0.0215#float(sys.argv[2])                                                        # Fit to match data from Williams (i.e. Ca = 13, Output = 0.075)                                                             % Wacq but Scaled(1.14/1.26) Ratio by Rudolf 2004 uM/ms max uptake of MCU      
    ...
    def Main(self):
        ...
        # plt.show()
        output_data_folder_path = "MCU={:.8f}".format(V_MCU)
        io.savemat('4s_MCU'+output_data_folder_path+'.mat', {'X': X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
