...
class SlowTwitch():
    ...
    def I_inj(self, t, i):
        ...
        if (t >= 0 and t <= 0.6):
            I_inj = current[i]
        for j in np.arange(1,125*4):
            if (t >= (1000/125)*j and t <= (1000/125)*j + 0.6):
                I_inj = current[i]
...
#        plt.show()
      ...
      #  io.savemat('twitch.mat', mdict = {'V': V_all, 'Force': Force_all, 'Cait': Cait_all, 'Cai': Cai_all, 'Camt': Camt_all, 'Cam': Cam_all})#mdict={'X' : X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
        io.savemat('/scratch/16201854/125Hz_All_X.mat', mdict={'X': X})
        io.savemat('/scratch/16201854/125Hz_All_Flux.mat', mdict={'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0)})
