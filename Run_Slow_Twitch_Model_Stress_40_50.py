...
class SlowTwitch():
    ...
    global alpha_c, alpham_m, nstep, Kank, n_S, Kasr, Cam_thresh_T, Cam_thresh_B
  
    global C, BCL2_mit, Atg13, dt
    global k_c, k_sb, k_ra, k_a, k_da, k_rb
    global gamma_J, sigma_J, w_JNK0, w_JNKS
    global gamma_D, sigma_D, w_DAPK0, w_DAPKS
    global gamma_B, sigma_B, w_BCL2P0, w_BCL2PJNK
    global k_out, k_in
    global gamma_T, sigma_T, w_MTOR0, w_MTORCa
    global gamma_G, sigma_G, w_ATG130, w_ATG13MTOR
    global gamma_H, sigma_H, w_BH30, w_BH3S, w_BH3Ca, w_BECN1P0, w_BECN1PDAPK
    global gamma_C, gamma_L, sigma_L, k_casp, w_BECN1DAPK, BCL2T, IP3RT    
   ...
   # Ion Concentrations and Nernst Potentials (Sarcolemma)
    ...
    Cam_thresh_T = 1823.6                                                        # uM mPTP Ca Threshold Value (Calculated from 120*.0194*1000 - Ichas)
    Cam_thresh_B = 737.2                                                        # uM mPTP Ca Threshold Value (Calculated from 120*.0194*1000 - Ichas)
    ...
    V_ANT   = 0.1*8.123                                                            # WU uM/ms Rate constant of the adenine nucleotide translocator
    ...
    BCL2T = 3
    IP3RT = 1
    BCL2_mit = np.random.normal(0.1, 0.03, 100)
    w_BECN1P0 = -0.3
    ...
    def dALLdt(X, t, self, i):
        ...
        """
        V, n, hk, m, h, S, u, n_t, hk_t, m_t, h_t, S_t, c_0, o_0, c_1, o_1, c_2, o_2, c_3, o_3, c_4, o_4, f_Ca, Cai_t, Cai, CaSR_t, CaSR, CaCS_t, CaCS, CaATP_t, CaATP, MgATP_t, MgATP, Mg_t, Mg, ATP_t, ATP, ADP_t, ADP, CaT, CaCaT, D0, D1, D2, A1, A2, P, PSR, PCSR, Cam_t, Cam, NADHm_t, NADHm, ADPm_t, ADPm, ATPm_t, ATPm, Psi_t, Psi, ROS_t, ROS, Cao_t, Ko_t, Ki_t, Ko, Ki, Stress, Atphg, JNK, DAPK, BCL2P, MTOR, ATG13, BH3, CASP, BECN1T, BECN1P = X
        ...
        BCL2U = BCL2T - BCL2P
        BECN1U = BECN1T - BECN1P
        LIGT = IP3RT + BECN1U
        LIGF = max(0, LIGT-BCL2U)
        BECN1F = BECN1U*(LIGF/LIGT)
        
        dStressdt = k_sb - (k_rb + k_ra*Atphg)*Stress
        dAtphgdt = k_a*(BECN1F + BECN1P)*ATG13 - k_da*Atphg
        dJNKdt = gamma_J*(1/(1+np.exp(-sigma_J*(w_JNK0 + w_JNKS*Stress))) - JNK)
        dDAPKdt = gamma_D*(1/(1+np.exp(-sigma_D*(w_DAPK0 + w_DAPKS*Stress))) - DAPK)
        dBCL2Pdt = gamma_B*(BCL2T*(1/(1+np.exp(-sigma_B*(w_BCL2P0 + w_BCL2PJNK*JNK)))) - BCL2P)
        dMTORdt = gamma_T*((1/(1+np.exp(-sigma_T*(w_MTOR0 + w_MTORCa*(Cai/3.8))))) - MTOR)
        dATG13dt = gamma_G*((1/(1+np.exp(-sigma_G*(w_ATG130 + w_ATG13MTOR*MTOR)))) - ATG13)
        dBH3dt = gamma_H*((1/(1+np.exp(-sigma_H*(w_BH30 + w_BH3Ca*(Cai/3.8) + w_BH3S*Stress)))) - BH3)

        
        # CASP represents Apoptosis. BH3 production driven primarily by calcium.
        if BCL2_mit[i] >= BH3:
            dCASPdt = gamma_C*(0 - CASP)
        if BH3 > BCL2_mit[i]:
            dCASPdt = gamma_C*(1-CASP)
    
        dBECN1Tdt = -k_casp*CASP*BECN1T
        dBECN1Pdt = gamma_L*(BECN1U*(1/(1+np.exp(-sigma_L*(w_BECN1P0 + w_BECN1PDAPK*DAPK)))) - BECN1P)

        return dVdt, dndt, dhkdt, dmdt, dhdt, dSdt, dudt, dntdt, dhktdt, dmtdt, dhtdt, dStdt, dc0dt, do0dt, dc1dt, do1dt, dc2dt, do2dt, dc3dt, do3dt, dc4dt, do4dt, dfCadt, dCaitdt, dCaidt, dCaSRtdt, dCaSRdt, dCaCStdt, dCaCSdt, dCaATPtdt, dCaATPdt, dMgATPtdt, dMgATPdt, dMgtdt, dMgdt, dATPtdt, dATPdt, dADPtdt, dADPdt, dCaTdt, dCaCaTdt, dD0dt, dD1dt, dD2dt, dA1dt, dA2dt, dPdt, dPSRdt, dPCSRdt, dCamtdt, dCamdt, dNADHmtdt, dNADHmdt, dADPmtdt, dADPmdt, dATPmtdt, dATPmdt, dPsitdt, dPsidt, dROStdt, dROSdt, dCaotdt, dKotdt, dKitdt, dKodt, dKidt, dStressdt, dAtphgdt, dJNKdt, dDAPKdt, dBCL2Pdt, dMTORdt, dATG13dt, dBH3dt, dCASPdt, dBECN1Tdt, dBECN1Pdt
    ...
    def Main(self):
        ...
        for i in range(40, 50):
            ...
            FCA_all.append(f_Ca)
            CAI_all.append(Cai)
            CASR_all.append(CaSR)
            ATP_all.append(ATP)
            ADP_all.append(ADP)
            CACAT_all.append(CaCaT)
            A2_all.append(A2)
            P_all.append(P)
            PCSR_all.append(PCSR)
            CAM_all.append(Cam)
            NADHM_all.append(NADHm)
            ADPM_all.append(ADPm)
            ATPM_all.append(ATPm)
            PSI_all.append(Psi)
            ROS_all.append(ROS)
            KOT_all.append(Ko_t)
            JMCU_all.append(JMCU)
            JNCX_all.append(JNCX)
            JMPTP_all.append(JMPTP)
            JF1F0_all.append(JF1F0)
            JANT_all.append(JANT)
            JAGC_all.append(JAGC)
            JPDH_all.append(JPDH)
            JETC_all.append(JETC)
            JSERCA_all.append(JSERCA)
            INAKT_all.append(INAK_t)
            STRESS_all.append(Stress)
            CASP_all.append(CASP)
...
#        plt.figure(figsize=(8,6))
#        plt.subplot(1,2,1)
#        plt.plot(self.t, CASP)
#        plt.subplot(1,2,2)
#        plt.plot(self.t, S)
#        
#        plt.show()
        ECC = io.savemat('/scratch/16201854/ECC_40_50.mat', mdict = {'V': V_all, 'RyR': RYR_all, 'FCA': FCA_all, 'CAI': CAI_all, 'CASR': CASR_all, 'CACAT': CACAT_all, 'CAM': CAM_all, 'JMCU': JMCU_all, 'JNCX': JNCX_all, 'JMPTP': JMPTP_all})#mdict={'X' : X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
        ATP = io.savemat('/scratch/16201854/ATP_40_50.mat', mdict = {'ATP': ATP_all, 'ADP': ADP_all, 'ATPM': ATPM_all,'ADPM': ADPM_all, 'P': P_all, 'PCSR': PCSR_all, 'KOT': KOT_all, 'INAKT': INAKT_all, 'JSERCA': JSERCA_all})#mdict={'X' : X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
        Mito = io.savemat('/scratch/16201854/Mito_40_50.mat', mdict = {'NADHM': NADHM_all, 'JETC': JETC_all, 'JPDH': JPDH_all, 'PSI': PSI_all,'JANT': JANT_all, 'JF1F0': JF1F0_all, 'AGC': JAGC_all})#mdict={'X' : X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
        Apoptosis = io.savemat('/scratch/16201854/Apoptosis_40_50.mat', mdict = {'STRESS': STRESS_all, 'CASP': CASP_all, 'ROS': ROS_all, 'FORCE': A2_all})#mdict={'X' : X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})

        return ECC, ATP, Mito, Apoptosis

        return Stress, Apoptosis, Force, Cai
      #  io.savemat('1Hz_Steady_State.mat', mdict={'X' : X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
        #io.savemat('40Hz_kETC_10PC_10s.mat', mdict={'Force': X[:,46], 'Ca': X[:,25]})#'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0)})
