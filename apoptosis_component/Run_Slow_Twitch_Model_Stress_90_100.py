...
class SlowTwitch():
    ...
    def I_inj(self, t, i):
        ...
        current[i] = 120 + 1.5*i #(Twitch)
        I_inj = 0
        # if (t >= 0 and t <= 0.6):
            # I_inj = current[i]
            
        for j in np.arange(1,25*4):
            if (t >= 0 and t <= 0.6):
                I_inj = current[i]
            if (t >= (1000/25)*j and t <= (1000/25)*j + 0.6):
                I_inj = current[i]
