...
class SlowTwitch():
    ...
    def I_inj(self, t):
        ...
        for i in np.arange(1,40*4):
            if (t >= 0 and t <= 0.6):
                I_inj = current
            if (t >= (1000/40)*i and t <= (1000/40)*i + 0.6):
                I_inj = current
   ...
   # Ion Concentrations and Nernst Potentials (Sarcolemma)
    ...
    V_MCU      = 0.0215#float(sys.argv[2])                                                        # Fit to match data from Williams (i.e. Ca = 13, Output = 0.075)                                                             % Wacq but Scaled(1.14/1.26) Ratio by Rudolf 2004 uM/ms max uptake of MCU      
    ...
    V_F1F0  = 0.05*3.6#float(sys.argv[2]) #3.6                                                              # Scaled til Psi 190 rest uM/ms Rate constant of the F1FO ATPase
    KATP    = 3.5                                                              # WU uM
    ...
    def I(self, t, V, n, hk, m, h, S, Ko, Ki):
        return -self.I_inj(t) + self.I_KDR(V, n, hk, Ko, Ki) + self.I_KIR(V, Ko, Ki) + self.I_Na(V, m, h, S) + self.I_Cl(V) + self.I_NaK(V, Ko) 
    ...
    def dALLdt(X, t, self):
        ...
        """
        V, n, hk, m, h, S, u, n_t, hk_t, m_t, h_t, S_t, c_0, o_0, c_1, o_1, c_2, o_2, c_3, o_3, c_4, o_4, f_Ca, Cai_t, Cai, CaSR_t, CaSR, CaCS_t, CaCS, CaATP_t, CaATP, MgATP_t, MgATP, Mg_t, Mg, ATP_t, ATP, ADP_t, ADP, CaT, CaCaT, D0, D1, D2, A1, A2, P_t, P, PSR, PCSR, Cam_t, Cam, NADHm_t, NADHm, ADPm_t, ADPm, ATPm_t, ATPm, Psi_t, Psi, ROS_t, ROS, Cao_t, Ko_t, Ki_t, Ko, Ki = X


            
        
        
        dVdt   = -(1.0/C_m) * (self.I(t, V, n, hk, m, h, S, Ko, Ki) + self.I_trans(V,u))    
        ...
        dPtdt     = 0.001*(self.J_HYD_t(ATP_t, ROS_t, Cai_t)/V_t) - tATP*(P_t - P)/V_t
        dPdt      = 0.001*(self.J_HYD(u, ATP, ROS, Cai, Ko_t)/V_m) + tATP*(P_t - P)/V_m + 0.001*(h0*A1 - hp*A2) - bbp*P - kp*(P - PSR)/V_m   
        if PSR*0.001*CaSR >= PP:
        ...
        return dVdt, dndt, dhkdt, dmdt, dhdt, dSdt, dudt, dntdt, dhktdt, dmtdt, dhtdt, dStdt, dc0dt, do0dt, dc1dt, do1dt, dc2dt, do2dt, dc3dt, do3dt, dc4dt, do4dt, dfCadt, dCaitdt, dCaidt, dCaSRtdt, dCaSRdt, dCaCStdt, dCaCSdt, dCaATPtdt, dCaATPdt, dMgATPtdt, dMgATPdt, dMgtdt, dMgdt, dATPtdt, dATPdt, dADPtdt, dADPdt, dCaTdt, dCaCaTdt, dD0dt, dD1dt, dD2dt, dA1dt, dA2dt, dPtdt, dPdt, dPSRdt, dPCSRdt, dCamtdt, dCamdt, dNADHmtdt, dNADHmdt, dADPmtdt, dADPmdt, dATPmtdt, dATPmdt, dPsitdt, dPsidt, dROStdt, dROSdt, dCaotdt, dKotdt, dKitdt, dKodt, dKidt
    ...
    def Main(self):
        ...
        Cai_t_0   = 0.1
        Cai_0     = 0.1
        CaSR_t_0  = 1500.0#1710.0
        CaSR_0    = 1500.0#1710.0
        CaCS_t_0  = 16900.0
        CaCS_0    = 16900.0
        CaATP_t_0 = 0.4
        CaATP_0   = 0.4
        ...
        X    = odeint(self.dALLdt, [V_0, n_0, hk_0, m_0, h_0, S_0, u_0, n_t_0, hk_t_0, m_t_0, h_t_0, S_t_0, c_0_0, o_0_0, c_1_0, o_1_0, c_2_0, o_2_0, c_3_0, o_3_0, c_4_0, o_4_0, f_Ca_0, Cai_t_0, Cai_0, CaSR_t_0, CaSR_0, CaCS_t_0, CaCS_0, CaATP_t_0, CaATP_0, MgATP_t_0, MgATP_0, Mg_t_0, Mg_0, ATP_t_0, ATP_0, ADP_t_0, ADP_0, CaT_0, CaCaT_0, D0_0, D1_0, D2_0, A1_0, A2_0, P_t_0, P_0, PSR_0, PCSR_0, Cam_t_0, Cam_0, NADHm_t_0, NADHm_0, ADPm_t_0, ADPm_0, ATPm_t_0, ATPm_0, Psi_t_0, Psi_0, ROS_t_0, ROS_0, Cao_t_0, Ko_t_0, Ki_t_0, Ko_0, Ki_0], self.t, args=(self,))
        ...
        # plt.show()
        output_data_folder_path = "F1F0={:.8f}".format(V_F1F0)
        io.savemat('4s_F1F0'+output_data_folder_path+'.mat', {'X': X, 'Flux' : (ICA_t, JRYR, JSERCAT, JSERCA, JMCUT, JMCU, JNCXT, JNCX, JMPTPT, JMPTP, JANTT, JANT, JF1F0T, JF1F0, JETCT, JETC, JPDHT, JPDH, JAGCT, JAGC, NADM_t, NADM, JHleak_t, JHleak, Eh_t, Eh, JHYD_t, JHYD)})
